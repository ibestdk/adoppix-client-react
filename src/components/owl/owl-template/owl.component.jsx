...
import OwlCard from "./owl-card/owl-card";
...
const OwlTemplate = () => {
  ...
  return (
    <div className=" mt-5 p-5 container m-auto bg-adoplight dark:bg-adopsoftdark rounded-lg">
      <div>
        <h2 className="text-adopsoftdark text-center text-2xl font-bold dark:text-adoplight duration-300">Owl Template</h2>
      </div>
      <div className="mt-3">
        <OwlCarousel {...options}>
          {pendingLists.map((card, cardIndex) => (
           <OwlCard key={cardIndex} card={card} />
          ))}
        </OwlCarousel>
      </div>
    </div>
