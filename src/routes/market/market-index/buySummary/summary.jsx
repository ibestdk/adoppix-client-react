      ...
      <div className="flex space-x-5">
        ...
        <div>
          <div className="bg-adopsoftdark w-[340px] p-5 rounded-lg flex flex-col justify-between">
            ...
            {productDatas.length > 0 ? (
              productDatas.map((data, index) => (
                <div key={index} className="flex justify-between text-xl items-end">
            ...
            <div className="py-4">
              ...
              <div className="flex justify-between text-xl mt-2">
                <div>เงินคงเหลือ</div>
                <div>
                {parseFloat(userMoney) >= (productDatas.length > 0
                  ? productDatas.reduce((total, data) => total + parseFloat(data.price), 0)
                  : productDatas.price) ? (
                  <div className="flex justify-between text-xl mt-2">
                    <div>{(parseFloat(userMoney) - (productDatas.length > 0 ? productDatas.reduce((total, data) => total + parseFloat(data.price), 0) : productDatas.price)).toFixed(2)}</div>
                  </div>
                ) : (
